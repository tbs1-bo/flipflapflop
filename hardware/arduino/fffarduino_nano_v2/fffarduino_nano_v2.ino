...
#define DELAY 80 // 80 µs bei 48 V, 200 µs bei 24 V

#define BUFFERSIZE 126*16/7 // number of bytes for buffer
byte buffer[BUFFERSIZE];
byte old_buffer[BUFFERSIZE];
byte width = 126;
byte height = 16;
int fullpic = 0;
int z = 0;  // show() stoped at buffer[z]
#define FP 1
...
void show() {
   ...
   */
  int used_buffer_size = width * height / 7;
  for(int i = z; i < used_buffer_size; i++) { // show() starts again at buffer[z]
    if(not (buffer[i] == old_buffer[i]) ) {
      for(int j = 6; j >= 0; j--) {
        if( not (((1<<j) & buffer[i]) == ((1<<j) & old_buffer[i])) ) {
          int x = (i*7 + 6 - j) % width;
          int y = (i*7 + 6 - j) / width;
    ...
    if(Serial.available() > 0) {
        z = i;                    // next show() will start at buffer[z]
        return;                   // return from show() when new data is available
    }
  }
  z = 0;
}
